...
service::AwaitableResolver Query::resolveUser(service::ResolverParams&& params) const
{
	...
	return service::ModifiedResult<UserQuery>::convert<service::TypeModifier::Nullable>(std::move(result), std::move(params));
}
...
{
	typeQuery->AddFields({
		schema::Field::Make(R"gql(User)gql"sv, R"md()md"sv, std::nullopt, schema->LookupType(R"gql(UserQuery)gql"sv))
	});
}
